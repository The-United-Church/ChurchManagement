    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-gray-400 dark:text-zinc-500 mt-0.5 flex items-center gap-2">
...
const STATUS_TONE: Record<ServiceMetric['status'], { bg: string; text: string; dot: string; label: string }> = {
  ...
  monitored: { bg: 'bg-gray-100/50 dark:bg-zinc-800/50 border-gray-200 dark:border-zinc-700', text: 'text-gray-500 dark:text-zinc-400', dot: 'bg-gray-300 dark:bg-zinc-500', label: 'Monitored' },
};
...
const ServiceCard: React.FC<{ service: ServiceMetric }> = ({ service }) => {
  ...
  return (
    <div className="rounded-xl border border-gray-100 dark:border-zinc-800 bg-gray-50/40 dark:bg-zinc-900/40 p-4 hover:border-gray-200 dark:hover:border-zinc-700 transition-colors">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2">
          <div className="rounded-md p-1.5 bg-gray-100/60 dark:bg-zinc-800/60">
            <Icon className="h-4 w-4 text-gray-600 dark:text-zinc-300" />
          </div>
          <div>
            <div className="text-sm font-semibold text-gray-900 dark:text-zinc-100">{service.name}</div>
            <div className="text-[10px] text-gray-400 dark:text-zinc-500 mt-0.5">{service.description}</div>
          </div>
        </div>
      ...
      <div className="flex items-end justify-between gap-2">
        <div>
          <div className="text-xl font-semibold text-gray-900 dark:text-zinc-100 tabular-nums">
            {service.responseMs > 0 ? `${service.responseMs}` : '—'}
            <span className="text-xs text-gray-400 dark:text-zinc-500 ml-1 font-normal">ms</span>
          </div>
          <div className="text-[10px] text-gray-400 dark:text-zinc-500 mt-0.5">Latency · last poll</div>
          {service.detail && (
            <div className={`text-[10px] mt-1 max-w-[180px] truncate ${service.status === 'down' ? 'text-red-400' : 'text-gray-400 dark:text-zinc-500'}`}>
              {service.detail}
            </div>
          )}
        </div>
...
const StatTile: React.FC<{ icon: React.ElementType; label: string; value: number | string }> = ({ icon: Icon, label, value }) => (
  <div className="rounded-xl border border-gray-100 dark:border-zinc-800 bg-gray-50/40 dark:bg-zinc-900/40 p-4">
    <div className="flex items-center justify-between">
      <span className="text-[11px] uppercase tracking-wider text-gray-400 dark:text-zinc-500 font-medium">{label}</span>
      <Icon className="h-3.5 w-3.5 text-gray-400 dark:text-zinc-600" />
    </div>
    <div className="text-2xl font-semibold text-gray-900 dark:text-zinc-100 tabular-nums mt-1">{value}</div>
  </div>
);
...
const CronJobsCard: React.FC = () => {
  ...
  return (
    <div className="lg:col-span-1 rounded-xl border border-gray-100 dark:border-zinc-800 bg-gray-50/40 dark:bg-zinc-900/40 p-5">
      <div className="flex items-center gap-2 mb-4">
        <CalendarIcon className="h-4 w-4 text-gray-400 dark:text-zinc-500" />
        <h3 className="text-sm font-semibold text-gray-800 dark:text-zinc-200">Scheduled Jobs</h3>
      </div>
      <div className="space-y-3">
        {jobs.map((j) => (
          <div key={j.name} className="rounded-lg border border-gray-100 dark:border-zinc-800 bg-gray-50/40 dark:bg-zinc-900/40 p-3">
            <div className="flex items-center justify-between mb-1.5">
              <span className="font-mono text-sm text-gray-900 dark:text-zinc-100">{j.name}</span>
            ...
            <p className="text-[11px] text-gray-400 dark:text-zinc-500 mb-2">{j.description}</p>
            <div className="flex items-center gap-3 text-[10px] font-mono text-gray-400 dark:text-zinc-600">
              <span className="bg-gray-100/60 dark:bg-zinc-800/60 border border-gray-100 dark:border-zinc-800 rounded px-1.5 py-0.5">{j.schedule}</span>
    ...
    return (
      <div className="lg:col-span-2 rounded-xl border border-gray-100 dark:border-zinc-800 bg-gray-50/40 dark:bg-zinc-900/40 p-5">
        <h3 className="text-sm font-semibold text-gray-800 dark:text-zinc-200 mb-4 flex items-center gap-2">
          <Shield className="h-4 w-4 text-gray-400 dark:text-zinc-500" /> Permission Matrix
        </h3>
        <p className="text-sm text-gray-400 dark:text-zinc-500 text-center py-8">No roles defined</p>
      </div>
    );
  }
  ...
  return (
    <div className="lg:col-span-2 rounded-xl border border-gray-100 dark:border-zinc-800 bg-gray-50/40 dark:bg-zinc-900/40 p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-semibold text-gray-800 dark:text-zinc-200 flex items-center gap-2">
          <Shield className="h-4 w-4 text-gray-400 dark:text-zinc-500" /> Permission Matrix
        </h3>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-gray-100 dark:border-zinc-800">
              <th className="text-left px-2 py-2 sticky left-0 bg-gray-50/40 dark:bg-zinc-900/40 text-[10px] uppercase tracking-wider text-gray-400 dark:text-zinc-500 font-semibold">Permission</th>
              {roles.map((r) => (
                <th key={r.id} className="px-2 py-2 text-center font-medium capitalize text-gray-800 dark:text-zinc-200 whitespace-nowrap">
                  {r.name.replace('_', ' ')}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {perms.map((p) => (
              <tr key={p} className="border-b border-gray-100/50 dark:border-zinc-800/50 hover:bg-gray-50/40 dark:hover:bg-zinc-900/40 group">
                <td className="text-left px-2 py-2 font-mono text-[11px] text-gray-500 dark:text-zinc-400 sticky left-0 bg-gray-50/40 dark:bg-zinc-900/40 group-hover:bg-gray-50 dark:group-hover:bg-zinc-900">
                  {p}
                </td>
                {roles.map((r) => {
                  ...
                  return (
                    <td key={r.id} className="px-2 py-2 text-center">
                      {has ? (
                        ...
                        <XCircle className="h-3.5 w-3.5 text-gray-300 dark:text-zinc-700 inline" />
                      )}
                    </td>
                  );
                })}
